...
const FALLBACK_CATEGORIES = [
  { id: 'lifting', label: '레이저 리프팅', emoji: '✨', color: 'border-purple-300 bg-purple-50',
    items: ['슈링크 유니버스','울쎄라','써마지 FLX','인모드','올리지오'] },
  { id: 'botox', label: '보톡스/윤곽주사', emoji: '💉', color: 'border-blue-300 bg-blue-50',
    items: ['보톡스','윤곽주사'] },
  { id: 'filler', label: '필러/실리프팅', emoji: '🌙', color: 'border-indigo-300 bg-indigo-50',
    items: ['필러','실리프팅','스컬트라'] },
  { id: 'booster', label: '스킨부스터', emoji: '💧', color: 'border-cyan-300 bg-cyan-50',
    items: ['리쥬란 힐러','쥬베룩','물광주사','포텐자'] },
  { id: 'skincare', label: '피부관리', emoji: '🌿', color: 'border-green-300 bg-green-50',
    items: ['스케일링','아쿠아필','비타민관리','LED재생레이저'] },
  { id: 'whitening', label: '미백/색소', emoji: '⚡', color: 'border-amber-300 bg-amber-50',
    items: ['엑셀V','피코토닝','레이저토닝'] },
  { id: 'acne', label: '여드름/점제거', emoji: '🔬', color: 'border-rose-300 bg-rose-50',
    items: ['점 제거','여드름 치료'] },
];
        ...
        <DialogHeader className="px-5 pt-5 pb-3 border-b border-border sticky top-0 bg-background z-10">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={handleClose} className="rounded-full p-1 hover:bg-accent transition-colors" aria-label="닫기">
          ...
          <div className="flex gap-1 mt-2">
            {Array.from({ length: totalSteps }).map((_, i) => (
              <div key={i} className={cn('h-1 flex-1 rounded-full transition-colors', i < step ? 'bg-info' : 'bg-muted')} />
            ))}
          </div>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {step === 2 && (
            <div>
              ...
              />
              {clinicAddress && (
                <p className="text-xs text-muted-foreground mt-2 px-1">📍 {clinicAddress}</p>
              )}
            </div>
          )}
            ...
            <div>
              ...
              <div className="mt-4 rounded-2xl bg-accent/30 p-4 space-y-2">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span>{format(dateObj, 'yyyy년 M월 d일 (EEEE)', { locale: ko })} {time}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
